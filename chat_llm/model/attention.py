...
def _resolve_use_fa3():
    ...
    if HAS_FA3:
        ...
        from chat_llm.utils.common import COMPUTE_DTYPE
...
def _sdpa_attention(
    ...
    return F.scaled_dot_product_attention(
        q, k, v, attn_mask=mask, dropout_p=0.0, is_causal=False, enable_gqa=use_gqd
    )


def flash_attn_func(q, k, v, causal: bool = False, window_size=(-1, -1)):
    """
    q ,k , v : (B, T, n_heads, head_dim)
    """
    if USE_FA3:
    ...
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    use_gqa = q.size(1) != k.size(1)
    out = _sdpa_attention(q, k, v, window_size, use_gqd=use_gqa)
...
flash_attn = SimpleNamespace(flash_attn_func=flash_attn_func, flash_attn_with_kvcache=flash_attn_with_kvcache)
