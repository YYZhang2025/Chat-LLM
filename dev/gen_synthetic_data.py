...
knowledge_path = os.path.join(os.path.dirname(__file__), "self_knowledge.md")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--append", action="store_true", help="Append to existing file instead of overwriting"
    )
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                completed_count += 1
                topic_short = (
                    metadata["topic"][:40] + "..." if len(metadata["topic"]) > 40 else metadata["topic"]
                )
